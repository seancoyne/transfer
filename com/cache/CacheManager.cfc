...
<cffunction name="init" hint="Constructor" access="public" returntype="CacheManager" output="false">
	...
	<cfscript>
		...
		ThreadPoolExecuter = getJavaLoader("#concPackage#ThreadPoolExecutor");
		...
		setDiscardQueueHandler(arguments.cacheFactory.getDiscardQueueHandler());
...
<cffunction name="add" hint="Adds a Transfer Object to the Pool" access="public" returntype="void" output="false">
	...
	<cfscript>
		...
		{
			className = transfer.getClassName();

			object = getObjectManager().getObject(className);
			cache = retrieveCache(className);
			key = JavaCast("string", getMethodInvoker().invokeMethod(transfer, "get" & object.getPrimaryKey().getName()));
...
<cffunction name="discardQueue" hint="Runs the aspect of clearing out discard queues" access="public" returntype="void" output="false">
	<cfscript>
