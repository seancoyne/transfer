...
<cffunction name="init" hint="Constructor" access="public" returntype="CacheManager" output="false">
	...
	<cfscript>
		...
		//loader = getJavaLoader().getURLClassLoader();

		TimeUnit = getJavaLoader().create("#concPackage#TimeUnit");
		...
		//setup a thread pool
		count = getObjectManager().getCachedClassCount();

		//if it's 0, we don't need it, and it throws an error
		if(count)
		{

			threadPool = getJavaLoader().create("#concPackage#ThreadPoolExecutor").init(JavaCast("int", 0),
																					JavaCast("int", count),
																					JavaCast("long", 60), //60 seconds timeout
																					TimeUnit.SECONDS,
																					getJavaLoader().create("#concPackage#SynchronousQueue").init(),
																					//silently discard if we hit our limits.
																					//use the $ sign for the inner class name
																					getJavaLoader().create("#concPackage#ThreadPoolExecutor$DiscardPolicy").init()
																					);
...
<cffunction name="add" hint="Adds a Transfer Object to the Pool" access="public" returntype="void" output="false">
	...
	<cfscript>
		...
		if(StructKeyExists(local, "transfer"))
		{
			...
			//_log("adding: #className# :: #key# :: #instance.sys.identityHashCode(local.transfer)#");
...
<cffunction name="have" hint="Checks if the Transfer is persistent in this" access="public" returntype="boolean" output="false">
	...
	<cfscript>
		...
		//_log("has: #arguments.class# :: #arguments.key# :: #cache.has(arguments.class, arguments.key)#");
...
<cffunction name="get" hint="gets a TransferObject from the pool" access="public" returntype="transfer.com.TransferObject" output="false">
	...
	<cfscript>
		...
		//_log("get: #arguments.class# :: #arguments.key#");
...
<cffunction name="discard" hint="removes a transfer from the cache" access="public" returntype="void" output="false">
	...
	<cfif cache.has(class, key)>
		...
		<cfscript>
			...
			{
				//_log("discard - discard: #class# :: #key#");
		...
		<cflock name="CacheManager.retrievecache.#arguments.class#" timeout="60" throwontimeout="true">
			<cfscript>
				if(NOT facade.hasCacheManager())
				{
					...
					{
						facade.setCacheManager(getJavaLoader().create("com.compoundtheory.objectcache.CacheManager").init(cacheConfig, getThreadPool()));
					}
				}
			</cfscript>
		</cflock>
