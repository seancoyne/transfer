...
<cffunction name="init" hint="Constructor" access="public" returntype="FacadeFactory" output="false">
	...
	<cfscript>
		setApplicationFacade(createObject("component", "transfer.com.facade.ApplicationFacade").init());
		setInstanceFacade(createObject("component", "transfer.com.facade.InstanceFacade").init());
		setRequestFacade(createObject("component", "transfer.com.facade.RequestFacade").init());
		setServerFacade(createObject("component", "transfer.com.facade.ServerFacade").init(arguments.version));
		setSessionFacade(createObject("component", "transfer.com.facade.SessionFacade").init());
		setNoneFacade(createObject("component", "transfer.com.facade.NoneFacade").init());
...
<cffunction name="getFacadeByScope" hint="Returns the right facade depending on what scope you enter" access="public" returntype="AbstractBaseFacade" output="false">
	...
	<cfscript>
		if(arguments.scope eq "instance")
		{
		...
		else if(ListFindNoCase("session,transaction", arguments.scope))
		{
			return getSessionFacade();
		}
		else if(arguments.scope eq "request")
		{
			return getRequestFacade();
		}
		else if(arguments.scope eq "none")
		{
			return getNoneFacade();
		}
		else if(arguments.scope eq "server")
		{
