...
<cffunction name="init" hint="Constructor" access="public" returntype="ObjectPool" output="false">
	<cfargument name="caseSensitive" hint="whether or not the cache is case sensitive or not, defaults to false" type="boolean" required="No" default="false">
	<cfscript>
		if(arguments.caseSensitive)
		{
			//use a hashmap which is case sensitive
			instance.pool = createObject("java", "java.util.Collections").synchronizedMap(createObject("java", "java.util.HashMap").init());
		}
		else
		{
