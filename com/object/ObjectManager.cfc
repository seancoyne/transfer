		...
		<cflock name="transfer.ObjectManager.getObject.#arguments.class#" timeout="60" throwontimeout="true">
			<cfif NOT StructKeyExists(cache, arguments.class)>
				<cfscript>
					...
					//do the loadParents here, so the child.notifyComplete() happens after.
					loadParents(arguments.class);
				...
				<cftry>
					...
					<cfset object.validate() />
					<cfcatch>
						<cfset StructDelete(cache, arguments.class) />
						<!--- if validation fails, remove it from the cache, and rethrow --->
						<cfrethrow>
					</cfcatch>
				</cftry>
			</cfif>
	...
	<cfif NOT StructKeyExists(cache, key)>
		<cflock name="transfer.ObjectManager.getObjectLazyManyToOne.#key#" timeout="60" throwontimeout="true">
			<cfscript>
				...
				{
					//clone object
					object = getObject(arguments.class).clone();
...
<cffunction name="getObjectLazyManyToMany" hint="creates an Object with only a single manytomany, with it's lazy attribute set to true" access="public" returntype="Object" output="false">
	...
	<cfif NOT StructKeyExists(cache, key)>
		<cflock name="transfer.ObjectManager.getObjectLazyManyToMany.#key#" timeout="60" throwontimeout="true">
			<cfscript>
				//get the defintion
				if(NOT StructKeyExists(cache, key))
				{
...
<cffunction name="loadParents" hint="loads the parents of a given class" access="public" returntype="void" output="false">
	<cfargument name="className" hint="" type="any" required="Yes">
	<cfscript>
		var OneToManyClasses =  getObjectGateway().getParentOneToManyClasses(arguments.className);
		var ManyToManyClasses = getObjectGateway().getParentManyToManyClasses(arguments.className);
		var len = ArrayLen(OneToManyClasses);
		var counter = 1;
		var object = 0;

		for(; counter lte len; counter = counter + 1)
		{
			object = getObject(OneToManyClasses[counter]);

			//if not complete at this stage, make it complete before the child loads
			if(NOT object.getIsComplete())
			{
				object.notifyComplete();
			}
		}

		len = ArrayLen(ManyToManyClasses);

		for(counter = 1; counter lte len; counter = counter + 1)
		{
			object = getObject(ManyToManyClasses[counter]);
