...
<cffunction name="init" hint="Constructor" access="public" returntype="JavaLoader" output="false">
	<cfargument name="serverFacade" hint="the server facade for the javaloader" type="transfer.com.facade.ServerFacade" required="Yes" _factory="transfer.com.facade.FacadeFactory" _factorymethod="getServerFacade">
	<cfscript>
	...
	<cfif NOT getServerFacade().hasJavaLoader()>
		<cflock name="transfer.server.JavaLoader" throwontimeout="true" timeout="60">
			<cfscript>
				...
				{
					getServerFacade().setJavaLoader(createObject("component", "transfer.com.util.javaloader.JavaLoader").init(queryJars()));
				}
			</cfscript>
		</cflock>
	</cfif>
