		...
		while(classIterator.hasNext())
		{
			...
			while(cIterator.hasNext())
			{
				...
				if(onetomany.getLink().getTo() eq class)
				{
					...
					arguments.buffer.writeLine("if(arguments.event.getTransferObject().getOriginalTransferObject().getParent" & arguments.object.getObjectName() & "isLoaded() AND arguments.event.getTransferObject().hasParent" & arguments.object.getObjectName() & "() AND arguments.event.getTransferObject().getParent" & arguments.object.getObjectName() &"().equalsTransfer(getThisObject())");
