...
<cffunction name="writeGetMemento" hint="Writes the getMemento function" access="public" returntype="void" output="false">
	...
	<cfscript>
		...
		if(primaryKey.getIsComposite())
		{
			...
			arguments.buffer.writeLine("memento." & primaryKey.getName() & " = StructNew();");
			...
			while(iterator.hasNext())
			{
				...
				arguments.buffer.writeLine("memento." & primaryKey.getName() & "." & property.getName() & " = get" & property.getName() & "();");
			}
			...
			while(iterator.hasNext())
			{
				...
				arguments.buffer.writeLine("{");
					arguments.buffer.writeLine("memento." & primaryKey.getName() & "." & manytoone.getName() & " = get" & manytoone.getName() & "().get"& manytoone.getLink().getToObject().getPrimaryKey().getName() &"();");
				arguments.buffer.writeLine("}");
			}
			...
			while(iterator.hasNext())
			{
				...
				arguments.buffer.writeLine("{");
					arguments.buffer.writeLine("memento." & primaryKey.getName() & ".parent" & parentObject.getObjectName() & " = getParent" & parentObject.getObjectName() & "().get"& parentObject.getPrimaryKey().getName() &"();");
				arguments.buffer.writeLine("}");
			}
		}
