	...
	<cfscript>
		...
		while(iterator.hasNext())
		{
			...
			arguments.buffer.writeLine(	"throw("& q() &"ManyToOneNotSetException"& q() &","&
										...
										q() & "In TransferObject '"& arguments.object.getClassName() &"' manytoone '"& manytoone.getLink().getTo() &"' does not exist."& q() &");");
			arguments.buffer.writeLine("}");
	...
	<cfscript>
		...
		while(iterator.hasNext())
		{
			...
			arguments.buffer.writeLine(	"throw("& q() &"OneToManyParentNotSetException"& q() &","&
										...
										q() & "In TransferObject '"& arguments.object.getClassName() &"' onetomany parent '"& parentObject.getClassName() &"' does not exist."& q() &");");
			arguments.buffer.writeLine("}");
			...
			arguments.buffer.cfscript(true);
				...
				*/
				...
				//arguments.buffer.writeLine("getParent" & parentObject.getObjectName() & "().get"& parentOneToMany.getName() & "Array();");
