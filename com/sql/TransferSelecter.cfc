...
<cffunction name="buildSelectSQL" hint="Creates the SQL for the Object BO" access="private" returntype="transfer.com.sql.Query" output="false">
	...
	<cfif NOT getQueryCache().hasQuery(key)>
		...
		<cfscript>
			...
			{
				columnStruct = buildColumnStruct(arguments.object); //meta data for the tables and columns
...
<cffunction name="buildColumnStruct" hint="Builds an struct with a key on table, with an array inside of each column, with type and column" access="private" returntype="struct" output="false">
	...
	<cfscript>
		...
		{
			/*
			if(NOT StructKeyExists(arguments.columnStruct, ""))
			{
				arguments.columnStruct[""] = ArrayNew(1);
			}
			//append it, but not to any table
			ArrayAppend(arguments.columnStruct[""], createColumnMeta(primaryKey));
			*/
		}
		else
		{
...
<cffunction name="flattenColumnStruct" hint="returns a flat array of column names, without duplicates" access="private" returntype="array" output="false">
	<cfargument name="columnStruct" hint="Struct of Arrays defining the columns" type="struct" required="yes">
	<cfscript>
		var flatColumnKeys = StructNew();
		var flatColumns = ArrayNew(1);
		var table = 0;
		var columnArray = 0;
		var len = 0;
		var counter = 0;
		...
		for(table in arguments.columnStruct)
		{
			columnArray = arguments.columnStruct[table];

			len = ArrayLen(columnArray);
			for(counter = 1; counter lte len; counter = counter + 1)
			{
				column = columnArray[counter];

				if(NOT StructKeyExists(flatColumnKeys, column.column))
				{
					ArrayAppend(flatColumns, column);
					StructInsert(flatColumnKeys, column.column, 1);
				}
			}
		}
